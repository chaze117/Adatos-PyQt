...
def generateCSJK(nev,ado,gyerekek):
    ...
    num_rows = len(gyerekek)+1      # Number of rows
    ...
    for gyerek in gyerekek:
        data.append([gyerek.nev,gyerek.ado,gyerek.taj,gyerek.a_nev,gyerek.cim,"1",'a',''])
    ...
    text.setFont("TNR", 13)
    text.textLines(f"""b. {len(gyerekek)} fő kedvezményezett eltartott után kívánom igénybe venni.""")
    c.drawText(text)
