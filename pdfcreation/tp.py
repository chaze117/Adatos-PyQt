...
def generateTP(nev,ado,taj,data):
    ...
    txtw = stringWidth(txt,fontName="TNRB",fontSize=12)
    text = c.beginText((PAGE_WIDTH - txtw) / 2.0, PAGE_HEIGHT-135)
    text.setFont("TNRB", 12)
    text.textLines(txt)
    c.drawText(text)
    ...
    text.textLines(txt)
    c.drawText(text)
    txt = f"igazolom, hogy {nev} -tól/től"
    txtw = stringWidth(txt,fontName="TNR",fontSize=15)
    ...
    txt = "Alulírott biztosított nyilatkozom, hogy az ügyintéző nevéről és hivatali elérhetőségéről a\nszóbeli tájékoztatást megkaptam."
